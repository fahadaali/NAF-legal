...
export default function Members() {
  ...
  const toggleActive = async (m: Member, reason = '') => {
    ...
    try {
      ...
      // النصّان المسجَّلان في naf-terms.md §١٠ · رسائل الصلاحية.
      // و`m.is_active` هي الحال قبل التبديل — فصدقُها يعني أننا نسحب.
      //
      // وصار للمنح نصّه بعد أن كان بلا نظير: التبليغ يقع في الحالين، وترْكُ
      // إحداهما بلا نصّ يجعل المسؤول يقرأ نجاحاً حيث لم يبلغ المركزَ خبر.
      if (r.reported === false) {
        setError(
          m.is_active
