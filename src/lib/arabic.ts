 ...
 */
/* **وكلُّ صنفٍ يقبل كلَّ ما يُطبَّع إليه، وإلا ضاعت المطابقة في الاتجاه
   الواحد.** `normalizeArabic` أعلاه يردّ `ئ` إلى `ي`، وكان صنفُ الياء
   `يى` بلا `ئ`: فيُطبَّع طرفُ الاستعلام «جزائي» إلى «جزايي» ثم يُبنى منه
   نمطٌ لا يقبل `ئ` أصلاً — فلا يجد النصَّ المكتوب «جزائي». وهو ما ظهر عند
   قياس بحث بنك البنود، ويمسّ بحث المنصة كلَّه.

   والقاعدة: مفاتيحُ هذا الجدول هي حروفُ ما بعد التطبيع، وقيمُه هي كلُّ
   مصادرها في `normalizeArabic` ومعها الحرفُ نفسه. */
