...
/**
 * أيُّ خطأٍ خرج من مسارٍ ولم يُمسَك.
 *
 * وبدونه يتولّاه معالجُ Hono الافتراضي: ٥٠٠ بجسم `text/plain` نصُّه
 * «Internal Server Error». و`req()` في الواجهة تحاول `res.json()` عليه فتفشل،
 * فترمي رسالتها العامّة «خطأ في الاتصال» — فيقرأ المستخدم عطلاً في شبكته
 * ويعيد الضغط، مهما كان السبب.
 *
 * وقد ظهر ذلك في حذف المستخدم: قيدُ مفتاحٍ أجنبيّ يرفض الحذف، فيُقرأ عطلاً
 * في الشبكة. ذاك أُصلح في موضعه، وهذا يمنع الصنف كلَّه.
 *
 * **والسبب يُسجَّل ولا يُعرض.** نصُّ الاستثناء من وقت التشغيل: قد يحمل اسم
 * جدولٍ أو شكلَ استعلام، ولا يقول لمن يقرؤه شيئاً يفعله. فالشاشة تأخذ جملةً
 * واحدة مسجَّلة، والسجلُّ يأخذ التفصيل.
 *
 * ومسارات `‎/api/*‎` وحدها تأخذ JSON: ما عداها أصولُ واجهة، وجسمُ JSON مكان
 * صفحةٍ لا يُقرأ.
 */
app.onError((err, c) => {
  console.error(`unhandled error on ${c.req.method} ${new URL(c.req.url).pathname}:`, err?.message ?? err);
  if (new URL(c.req.url).pathname.startsWith('/api/')) {
    return c.json({ error: 'تعذّر إتمام العملية. أعد المحاولة بعد قليل' }, 500);
  }
  return c.text('تعذّر تحميل الصفحة. أعد المحاولة بعد قليل', 500);
});
