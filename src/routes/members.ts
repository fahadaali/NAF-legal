// إعدادات الأدوار — عرض الأعضاء، وتغيير الصلاحية، وإيقاف العضو
//
// «كل ما بعد الباب تقرّره هذه المنصة من إعداداتها.» فالمركز لا يُسأل عن دور
// ولا يُخبَر به؛ ولا يُبلَّغ إلا بالإيقاف، ليظهر السبب للمستخدم في شبكته.
...
app.get('/', async (c) => {
  ...
  return c.json({
    members: (rows.results ?? []).map((m) => ({
      ...
      // ليعرف العميل أيّ صفٍّ هو صفّ القارئ نفسه فيمنع عنه أزرار الإيقاف
...
app.patch('/:id/active', async (c) => {
  ...
  if (id === me.memberId && !body.is_active) {
    return c.json({ error: 'لا يمكنك إيقاف نفسك' }, 400);
  }

  const reason = (body.reason ?? '').trim();
  // السبب يُعرض للمستخدم في شبكة المركز، فلا يُرسَل فارغاً عند الإيقاف.
  if (!body.is_active && !reason) {
    return c.json({ error: 'اكتب سبب الإيقاف — يُعرض للعضو في شبكته' }, 400);
  }
  ...
  // الإيقاف يسري محلياً فور كتابته: الوسيط يقرأ `is_active` في كل طلب،
  ...
  if (!body.is_active) {
    try {
      ...
      // والإيقاف قائم، والناقص إظهارُ سببه في شبكة المركز وحده.
