...
//
// المالك ينشئ رابطًا برمز؛ والمراجِع يفتحه فيعلّق ويعتمد — **وله حساب في
// المنصة**. كان الرابط يُفتح بلا حساب، وأُخضعت مسارات `/public/*` و
// `/review/:token` للدخول الموحّد بقرارٍ موصوفٍ في `audit/sso-report.md`.
//
// والبادئة `public` باقية في المسار: روابطُ مراجعةٍ أُرسلت تحمل `/review/:token`
// وهي تصل هذه المسارات، وإعادةُ التسمية تكسرها بلا مقابل. فهي اسمُ مسارٍ لا
// وصفُ حال — والحارس أدناه هو ما يقول الحال.
...
/* ── مسارات المراجِع ──
 *
 * لا `requireAuth` عليها ولا حاجة: وسيط الدخول الموحّد يعمل على كل مسار في
 * `index.ts` قبل أن يبلغها الطلب، فمن لا جلسة له لا يصل إلى هنا أصلاً.
 * والرمز يبقى شرطاً ثانياً: من دخل المنصة لا يقرأ مسوّدةً لم تُشارَك معه. */
...
app.post('/public/:token/decision', async (c) => {
  ...
  await c.env.DB.prepare('INSERT INTO share_comments (id, share_id, author, body, created_at) VALUES (?, ?, ?, ?, ?)')
    .bind(
      ...
      /* بلا رمزٍ تصويريّ: التعليق يُعرض في `ReviewPage` — واجهةً لا رسالةَ
         قناةٍ خارجية — وحظرُ الإيموجي في الواجهات (§3) لا استثناء له هنا.
         والفعل مقروءٌ من كلمته، والحالُ معروضةٌ في شارة أعلى الصفحة. */
      decision === 'approved' ? 'اعتمد المسودّة' : 'طلب تعديلات',
      Date.now()
    )
    .run();
  return c.json({ ok: true });
